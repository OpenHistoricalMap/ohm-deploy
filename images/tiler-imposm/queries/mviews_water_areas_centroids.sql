-- This script creates materialized views for water centroids for each zoom level.
...
BEGIN
    ...
    -- Create unique index on osm_id and type
    sql_unique_index := format('CREATE UNIQUE INDEX IF NOT EXISTS idx_%I_osm_id_type ON %I (osm_id, type);', view_name, view_name);
    EXECUTE sql_unique_index;
    RAISE NOTICE 'Created unique index: idx_%_osm_id_type', view_name;
