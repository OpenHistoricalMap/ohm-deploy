...
DECLARE
    ...
    tmp_mview    text;
BEGIN
    RAISE NOTICE '==> [MVIEW LINE] Creating % from % (simplify_tol: %m, min_metric: %s, where_filter: %s)', target, source, simplify_tol, min_metric, where_filter;
    ...
    -- 3) Apply length filter if requested
    IF min_metric > 0 THEN
        sql := sql || format(' AND ST_Length(geometry) >= %s', min_metric);
    END IF;
