 ...
 *
 * @param source_mview - Name of the source materialized view to read from (must contain area geometries)
 * @param target_mview - Name of the target materialized view to create/replace (will contain point geometries)
 * @param custom_filter - Additional WHERE clause filter (e.g., "type IN ('water', 'pond')")
 */
CREATE OR REPLACE FUNCTION create_mview_centroid_from_mview(
    source_mview      text,
    target_mview      text,
